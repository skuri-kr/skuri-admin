...
- 구현 완료:
  ...
  - 공개 채팅방 운영 목록/필터 + 상세/메시지 조회 + 관리자 생성/삭제
- 검증:
  - `npm run lint`
  - `npm run build`
...
- `/chat-rooms`는 `GET /v1/admin/chat-rooms*` 기준으로 PARTY 타입을 제외한 공개 채팅방 전체를 membership 없이 조회하는 운영 화면이다. 관리자 생성/삭제는 유지하고, 멤버 관리/공지성 시스템 메시지는 follow-up 범위로 남긴다.
- `/parties` 상세는 `GET /v1/admin/parties/{partyId}/messages`로 관리자 파티 채팅 이력을 membership 없이 조회할 수 있다.
